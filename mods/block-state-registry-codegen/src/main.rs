...
fn main() {
    if let Err(error) = run() {
        eprintln!("block state registry codegen failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mut package = "generated-block-state".to_string();
...
> {
    ...
    for dependency in dependencies.values() {
        ...
        let Some(metadata) = mod_manifest
            ...
            .and_then(|value| value.get("block_state"))
            .and_then(Value::as_table)
        else {
            continue;
        };
        ...
        if !ids.insert(id.clone()) {
            return Err(format!("duplicate block state id '{id}'").into());
        }
        if !fields.insert(field.clone()) {
            return Err(format!("duplicate block state field '{field}'").into());
        }
        let crate_ident = ty
            .split("::")
            .next()
...
fn required_string(
    ...
    table
        ...
        .and_then(Value::as_str)
        .map(str::to_string)
        .ok_or_else(|| format!("block state field '{key}' must be a string").into())
}
