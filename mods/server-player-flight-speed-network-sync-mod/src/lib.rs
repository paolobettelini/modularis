...
use player_flight_speed_network_message_types::PlayerFlightSpeedChanged;
...
impl ServerPlayerFlightSpeedNetworkSyncMod {
    pub fn init<S: ServerPlayerFlightSpeedApi, N: ServerNetworkEventsApi>(
        bevy: &mut BevyMod,
        _speed: &mut S,
        _network: &mut N,
...
fn sync_joined_players(
    speeds: Res<ServerPlayerFlightSpeeds>,
...
fn sync_flight_speed_changes(
    mut changed: MessageReader<ServerPlayerFlightSpeedChanged>,
    mut packets: MessageWriter<ServerPacketOut>,
) {
    for change in changed.read() {
        send_flight_speed(change.player_id, change.multiplier, &mut packets);
    }
}

fn send_flight_speed(
    ...
    packets.write(ServerPacketOut {
        ...
        message: ClientBoundMessage::PlayerFlightSpeedChanged(PlayerFlightSpeedChanged {
            multiplier,
        }),
    });
}
