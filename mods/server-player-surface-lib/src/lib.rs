...
pub enum ServerPlayerSurfaceSet {
    Observe,
}

#[derive(Clone)]
pub struct ServerSurfaceAnchor {
...
pub fn rebase(
    ...
    anchor: &ServerSurfaceAnchor,
    movement: &mut PendingServerPlayerMove,
) {
    let Some(claim) = movement.surface else {
        return;
    };
    let foot = Vec3::from_array(claim.local_foot);
    if !foot.is_finite()
        || anchor.epoch != movement.movement_epoch
        || claim.surface != anchor.frame.0.as_u128()
        || movement.current_position.distance_squared(anchor.world_foot) > 0.0001
    {
        return;
    }
    let Some(pose) = world.frames().transform(&anchor.scope, anchor.frame) else {
        return;
    };
