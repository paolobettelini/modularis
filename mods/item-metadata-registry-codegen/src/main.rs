...
fn main() {
    if let Err(error) = run() {
        eprintln!("item metadata registry codegen failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mut package = "generated-item-metadata".to_string();
...
fn collect_metadata(
    ...
    for dependency in dependencies.values() {
        ...
        let Some(metadata) = mod_manifest
            ...
            .and_then(|value| value.get("item_metadata"))
            .and_then(Value::as_table)
        else {
            continue;
        };
        ...
        if !ids.insert(id.clone()) {
            return Err(format!("duplicate item metadata id '{id}'").into());
        }
        if !fields.insert(field.clone()) {
            return Err(format!("duplicate item metadata field '{field}'").into());
        }
        let crate_ident = ty
            .split("::")
            .next()
    ...
    if declarations.is_empty() {
        return Err("item metadata registry requires at least one contributor".into());
    }
