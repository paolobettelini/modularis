...
use parkour_gameplay_lib::{ParkourConfig, ParkourFrameEdit, ParkourRun, ParkourUpdate};
...
use server_player_flight_api::SetPlayerFlightCapability;
use server_player_lifecycle_events_api::ServerPlayerLeft;
use server_player_registry_api::{ServerPlayerMovementApplied, ServerPlayerRegistry};
use server_player_scale_api::SetServerPlayerScale;
use server_player_surface_lib::ServerSurfaceAnchors;
...
#[derive(SystemParam)]
pub struct InstancePlayerPolicyWriters<'w> {
    gravity: MessageWriter<'w, SetServerPlayerGravity>,
    flight: MessageWriter<'w, SetPlayerFlightCapability>,
    scale: MessageWriter<'w, SetServerPlayerScale>,
}

#[derive(Component, Debug, Clone)]
pub struct TheCrownInstanceEntity {
...
pub fn start_instance(
    ...
    if runtime.instances.contains_key(&spec.instance_id) {
        return Ok(());
    }
    let parent = runtime
        .root
        .clone()
        .ok_or_else(|| "TheCrown root is not ready".to_owned())?;
    ...
    let entity = scopes
        .spawn(commands, descriptor)
        .map_err(|error| error.to_string())?;
    commands.entity(entity).insert(TheCrownInstanceEntity {
    ...
    info!(
        "started TheCrown {} instance '{}'",
        spec.mode, spec.instance_id
    );
    Ok(())
}
...
pub fn stop_instance(
    ...
    if !instance.players.is_empty() {
        return Err(format!(
            "instance still contains {} player(s)",
            instance.players.len()
        ));
    }
...
pub fn assign_admitted_players(
    ...
    mut policy_messages: InstancePlayerPolicyWriters,
    ...
    for admitted in admitted.read() {
        if runtime.players.contains_key(&admitted.player_id) {
            continue;
        }
        let Some(instance) = runtime.instances.get(&admitted.session.instance_id) else {
            warn!(
                "player {} authenticated for unavailable instance '{}'",
                admitted.player_id, admitted.session.instance_id
            );
        ...
        if instance.spec.mode != admitted.session.mode {
            warn!(
                "player {} transfer mode does not match instance",
                admitted.player_id
            );
            kicks.write(ServerKickRequested {
                target: ServerKickTarget::Player(admitted.player_id),
        ...
        let (player_scope, player_world, arena_entity, mut spawn) = match mode {
            GameMode::Hub => (
                ...
                shared_world.expect("Hub has a shared world"),
                None,
                shared_spawn,
            ),
            GameMode::Parkour => {
                let scope = ScopeNodeId::new(format!(
                    "thecrown:instance:{}:player:{}",
                    admitted.session.instance_id, admitted.player_id
                ));
                let entity = scopes
                    .spawn(
                        &mut commands,
                        ScopeNodeDescriptor::child(scope.0.clone(), instance_scope)
                            .with_facet(ScopeFacetId::visibility()),
                    )
                    .expect("parkour player scope must be unique");
                commands.entity(entity).insert(TheCrownPlayerArena {
                    player_id: admitted.player_id,
                });
                let world_id = WorldInstanceId::new(format!(
                    "thecrown:parkour:{}:player:{}",
                    admitted.session.instance_id, admitted.player_id
                ));
                scope_worlds
                    .bind(
        ...
        let previous = scopes
            .assign_player(admitted.player_id, player_scope.clone())
            .expect("player scope exists");
        scope_changes.write(ServerPlayerScopeChanged {
            player_id: admitted.player_id,
            previous,
            current: Some(player_scope.clone()),
        });
        ...
        policy_messages.gravity.write(SetServerPlayerGravity {
            player_id: admitted.player_id,
            gravity: player_policy.gravity,
        });
        policy_messages.flight.write(SetPlayerFlightCapability {
            player_id: admitted.player_id,
            enabled: player_policy.flight_enabled,
        });
        policy_messages.scale.write(SetServerPlayerScale {
            player_id: admitted.player_id,
        ...
        runtime
            .instances
            .get_mut(&admitted.session.instance_id)
            .expect("instance still exists")
            .players
            .insert(admitted.player_id);
        runtime.players.insert(
            admitted.player_id,
            PlayerRecord {
                instance_id: admitted.session.instance_id.clone(),
                scope: player_scope,
                world: player_world,
                arena_entity,
        ...
        messages.write(PublishServerChatMessage {
            audience: Audience::personal(admitted.player_id),
            text: match mode {
                GameMode::Hub => {
                    format!("Welcome to TheCrown Hub {}", admitted.session.instance_id)
                }
                GameMode::Parkour => {
                    "Welcome to TheCrown Parkour. Reach the next block without falling!".to_owned()
                }
            },
        });
...
pub fn progress_parkour(
    ...
    surface_anchors: Res<ServerSurfaceAnchors>,
    time: Res<Time>,
    mut runtime: ResMut<TheCrownRuntime>,
    ...
    for movement in movements.read() {
        ...
        let Some(entity) = arena.arena_entity else {
            continue;
        };
        ...
        // Consume the support selected by the authoritative movement solver.
        // Re-querying collision here used a later frame pose and intermittently
        // missed moving/tilted checkpoints at network tick boundaries.
        let supported_frame = movement
            .support_surface
            .map(VoxelFrameId::from_u128)
            .or_else(|| {
                surface_anchors
                    .0
                    .get(&movement.player_id)
            ...
            .filter(|frame| {
                run.blocks()
                    .iter()
                    .skip(1)
                    .any(|block| block.frame.id == *frame)
            });

        let now_seconds = time.elapsed_secs_f64();
        let update = if let Some(supported_frame) = supported_frame {
            run.observe_supported_frame(
                &runtime.config,
                movement.position,
                now_seconds,
                Some(supported_frame),
            )
        } else {
            // Some accepted packets have no final support marker (for example
            // when the client already resolved the landing locally). Sweep the
            // authoritative movement segment against only future checkpoint
            // frames as a deterministic fallback.
            run.observe_movement_with_frames(
                &runtime.config,
        ...
        if let Some(completed_score) = update.completed_score {
            let Some(player) = runtime.players.get_mut(&movement.player_id) else {
                continue;
            };
            if player.parkour_record.is_none() {
                player.pending_completed_score = Some(
                    player
                        .pending_completed_score
                        .unwrap_or(0)
                        .max(completed_score),
                );
        ...
        if update.score_changed && update.teleport.is_none() {
            sounds.write(PlayServerSound {
                ...
                playback: SoundPlayback::new(SoundId::NoteBlockBass)
                    .with_volume(1.0)
                    .with_pitch(0.9 + (update.combo - 1) as f32 * 0.05)
                    .at(movement.position.to_array()),
            });
        }
    }

    // A client may stop publishing movement immediately after its locally
    // resolved landing. Poll the final authoritative registry position so a
    // checkpoint cannot require another jump/input packet to be recognized.
    // The library checks only future frames, preventing duplicate scoring for
    // the platform that is now at index zero.
    let settled_players = runtime
        .players
        .iter()
        .filter_map(|(player_id, record)| {
            record
                .arena_entity
                .map(|entity| (*player_id, entity, record.world.clone()))
        })
        .collect::<Vec<_>>();
    let config = runtime.config.clone();
    for (player_id, entity, world_id) in settled_players {
        let Some(player) = players.player(player_id) else {
            continue;
        };
        let position = Vec3::from_array(player.position);
        if position.y < config.start.y as f32 - config.fall_reset_distance {
            continue;
        }
        let Ok(mut run) = runs.get_mut(entity) else {
            continue;
        ...
        let world_scope = parkour_world_scope(&world_id);
        let anchored_frame = surface_anchors
            .0
            .get(&player_id)
            .map(|anchor| anchor.frame)
            .filter(|frame| {
        ...
        let update = if let Some(frame) = anchored_frame {
            run.observe_supported_frame(
                &config,
                position,
            ...
            run.observe_position_with_frames(
                &config,
                position,
                time.elapsed_secs_f64(),
                |frame| world.frames().transform(&world_scope, frame),
        ...
        if !update.score_changed || update.teleport.is_some() {
            continue;
        }
        ...
        publish_score(player_id, &update, &mut messages);
        sounds.write(PlayServerSound {
            audience: Audience::personal(player_id),
            playback: SoundPlayback::new(SoundId::NoteBlockBass)
                .with_volume(1.0)
                .with_pitch(0.9 + (update.combo - 1) as f32 * 0.05)
                .at(position.to_array()),
        });
    }
}
...
pub fn apply_record_results(
    ...
    for result in loaded.read() {
        let Some(player) = runtime.players.get_mut(&result.player_id) else {
            continue;
        };
        match &result.result {
            Ok(record) => {
            ...
            Err(error) => warn!(
                "could not load parkour record for player {}: {error}",
                result.player_id
            ),
        }
    }
    ...
    for result in submitted.read() {
        ...
        match &result.result {
            ...
            Err(error) => {
                ...
                warn!(
                    "could not save parkour record for player {}: {error}",
                    result.player_id
                );
            }
        }
...
pub fn cleanup_left_players(
    ...
    for left in left.read() {
        let Some(player) = runtime.players.remove(&left.player_id) else {
            continue;
        };
        if let Some(instance) = runtime.instances.get_mut(&player.instance_id) {
            instance.players.remove(&left.player_id);
        }
...
fn apply_parkour_update(
    ...
    for frame_edit in &update.frame_edits {
        let ParkourFrameEdit::Spawn(plan) = frame_edit else {
            continue;
        };
...
fn publish_block_mutation(
    ...
    if mutation.current.block == BlockId::Air {
        if let Some(writer) = broken.as_mut() {
            writer.write(ServerBlockBroken {
                player_id,
        ...
        writer.write(ServerBlockPlaced {
            player_id,
            scope: mutation.scope,
            position: mutation.position,
